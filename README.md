...
2. **Bitwarden Secrets Manager (BSM)**: созданная организация и 7 заполненных секретов (см. таблицу ниже).
3. **cloud.ru S3**: созданный Object Storage bucket.
   - 💡 *15 ГБ бесплатно в рамках free-tier*
4. **Yandex Disk**: полученный OAuth-токен с правами на запись в папку `/backups/`.
5. **GitHub Token**: Personal Access Token (если вы клонируете приватный репозиторий).
...
### Секреты (Bitwarden Secrets Manager)
В BSM должны быть созданы следующие 7 секретов. `BW_ACCESS_TOKEN` передаётся только через env var.
...
### `scripts/secrets.py` — синхронизация с Bitwarden
**Что делает:** Синхронизирует секреты из BSM в локальный `.env` (chmod 600, root:root).  
**Когда использовать:** После изменения секретов в BSM, проверка доступности ключей.

```bash
BW_ACCESS_TOKEN=bws_token_xxx python3 scripts/secrets.py sync   # синхронизировать все
BW_ACCESS_TOKEN=bws_token_xxx python3 scripts/secrets.py list   # список ключей
BW_ACCESS_TOKEN=bws_token_xxx python3 scripts/secrets.py get cloudru/s3/access-key
```
